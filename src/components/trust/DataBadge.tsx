...
const badgeTone: Record<DataBadgeType, string> = {
  ...
  ESTIMATED: "border-amber-300/70 bg-amber-50 text-amber-900 dark:border-amber-300/30 dark:bg-amber-300/10 dark:text-amber-200",
...
export function DataBadge({ type, label, className = "" }: DataBadgeProps) {
  const displayLabel = label || (type === "LIVE" ? "PROVIDER" : type === "ESTIMATED" ? "ESTIMATED — NOT CONFIRMED" : type === "VERIFIED RECENTLY" ? "RECENT PROVIDER RESULT" : type);
  return (
