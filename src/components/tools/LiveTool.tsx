  ...
  return (
    <div className={softPanel("mx-auto max-w-5xl rounded-[32px] p-5")}>
      ...
      <form onSubmit={check} className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
        ...
        <input value={date} onChange={(event) => setDate(event.target.value)} type="date" min={minBookableDateIso()} max={maxBookableDateIso()} className="h-13 rounded-2xl border border-slate-200 bg-white px-4 font-bold dark:border-white/10 dark:bg-white/8 dark:text-white" />
